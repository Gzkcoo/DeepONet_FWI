...
def train_one_epoch(model, criterion, optimizer, lr_scheduler,
    ...
    for data, source, label in dataloader:
        ...
        if with_source == 1:
            ...
            output = model(data)
        loss, loss_g1v, loss_g2v, loss_perceptual = criterion(output, label)
        loss.backward()
        optimizer.step()
        ...
        train_perc += loss_perceptual_val

    train_l1 /= len(dataloader)
    train_mse /= len(dataloader)
    train_perc /= len(dataloader)
    t2 = default_timer()
    loss_history_MAE.append(train_l1)
    loss_history_MSE.append(train_mse)
    loss_history_PERC.append(train_perc)
    print('Train epoch {:d} , L1Loss = {:.6f}, MSELoss = {:.6f}, perceptualLoss = {:.6f}, using {:.6f}s'.format(
        epoch, train_l1, train_mse, train_perc, t2 - t1))
...
def evaluate(model, criterion, dataloader, device, with_source):
    ...
    with torch.no_grad():
        ...
        for data, source, label in dataloader:
            ...
            train_l1 += loss_g1v.item()
            train_mse += loss_g2v.item()
            train_perc += loss_perceptual.item()
    ...
    loss_history_val_MAE.append(train_l1)
    loss_history_val_MSE.append(train_mse)
    loss_history_val_PERC.append(train_perc)
    print('Test L1Loss = {:.6f}, MSELoss = {:.6f}, loss_perceptual = {:.6f}, using {:.6f}s'.format(
        train_l1, train_mse, train_perc, t2 - t1))
    return train_l1
...
def train(args):
    ...
    perceptualloss = network.PerceptualLoss().to(args.device)
    ...
    def criterion(pred, gt):
        ...
        loss_perceptual = perceptualloss(pred, gt)

        loss = args.lambda_g1v * loss_g1v + args.lambda_g2v * loss_g2v + args.lambda_perceptual * loss_perceptual
        return loss, loss_g1v, loss_g2v, loss_perceptual
    ...
    # plt.savefig('{}MAE.png'.format(args.train_out)) #nuaa
    ...
    ax31.semilogy(np.arange(1, args.epoch + 1), loss_history_PERC, label='train')
    ax31.semilogy(np.arange(1, args.epoch + 1), loss_history_val_PERC, label='test')
    ax31.set_xlabel('Epochs')
    ax31.set_ylabel('Loss')
    ax31.set_title('PerceptualLoss')
    ax31.legend()

    plt.savefig(os.path.join(args.train_out, "Perceptual.png"))
...
def parse_args():
    ...
    parser.add_argument('-t', '--train_anno', default='flatvel_b_train_L.txt', help='name of train anno')
    parser.add_argument('-v', '--val_anno', default='flatvel_b_val_L.txt', help='name of val anno')
    ...
    parser.add_argument('-g4v', '--lambda_perceptual', type=float, default=3)
