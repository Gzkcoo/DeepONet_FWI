...
def train_one_epoch(model, criterion, optimizer, lr_scheduler,
    ...
    t1 = default_timer()
    for data, frequency, source, label in dataloader:
        optimizer.zero_grad()
        data, frequency, source, label = data.to(device), frequency.to(device), source.to(device), label.to(device)
        if with_source == 1:
            source = torch.cat([source, frequency], dim=1)
...
def evaluate(model, criterion, dataloader, device, with_source):
    ...
    with torch.no_grad():
        ...
        for data, frequency, source, label in dataloader:
            ...
            frequency = frequency.to(device, non_blocking=True)
...
def train(args):
    ...
    dataset_train = FLDataset(
        ...
        transform_frequency=transform_frequency,
        du=args.data_url
    )

    print('Loading validation data')
    # Normalize data and label to [-1, 1]
    dataset_valid = FLDataset(
        args.val_anno,
        preload=True,
        sample_ratio=1,
        file_size=ctx['file_size'],
...
def parse_args():
    ...
    parser.add_argument('-ds', '--dataset', default='curvevel-a-FL', type=str, help='dataset name')
    ...
    parser.add_argument('-t', '--train_anno', default='curvevel_a_train_FL.txt', help='name of train anno')
    parser.add_argument('-v', '--val_anno', default='curvevel_a_val_FL.txt', help='name of val anno')
    ...
    parser.add_argument('--layer_sizes', nargs='+', type=int, default=[10, 256, 256, 256, 512], help='trunk_net layer sizes')
    # Training related
