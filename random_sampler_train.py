...
def train_one_epoch(model, criterion, optimizer, lr_scheduler,
                    dataloader, device, epoch, print_freq, writer, arr, xy_loc):
    global step
    model.train()
    ...
    for data, label in metric_logger.log_every(dataloader, print_freq, header):
        ...
        label2 = label.reshape(-1, 4900, 1)
        labels = torch.zeros((args.batch_size, args.N_sample, 1))   # (batch, 500, 1)
        loc_samples = torch.zeros((args.batch_size, args.N_sample, 2))  # (batch, 500, 2)
        for i in range(args.batch_size):
            arr_sample = random.sample(arr, args.N_sample)  # 随机坐标索引
            loc_sample = xy_loc[arr_sample, :]    # 网格中随机获取的坐标点
            lb = label2[i][arr_sample, :]
            loc_samples[i] = loc_sample
            labels[i] = lb


        data, labels, loc_samples = data.to(device), labels.to(device), loc_samples.to(device)
        output = model(data, loc_samples)
        loss, loss_g1v, loss_g2v = criterion(output, labels)
        loss.backward()
        optimizer.step()
...
def evaluate(model, criterion, dataloader, device, writer, xy_loc):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter='  ')
    header = 'Test:'
    ...
    with torch.no_grad():
        for data, label in metric_logger.log_every(dataloader, 20, header):
            xy_locs = torch.zeros((data.shape[0], 4900, 2))  # (batch, 4900, 2)
            for i in range(data.shape[0]):
                xy_locs[i] = xy_loc
            xy_locs = xy_locs.to(device)

            data = data.to(device, non_blocking=True)
            label2 = label.reshape(-1, 4900, 1).to(device, non_blocking=True)
            output = model(data, xy_locs)
            loss, loss_g1v, loss_g2v = criterion(output, label2)
...
def main(args):
    ...
    # Coordinate
    xc = torch.arange(0, 70)
    yc = torch.arange(0, 70)
    xm, ym = torch.meshgrid(xc, yc)
    x = xm.reshape(-1, 1)
    y = ym.reshape(-1, 1)
    xy_coordinate = torch.cat([x, y], dim=1).float()  # (4900, 2)
    xy_coordinate = xy_coordinate / 70  # 输入转换为（0，1）之间
    arr = [i for i in range(4900)]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(model, criterion, optimizer, lr_scheduler, dataloader_train,
                        device, epoch, args.print_freq, train_writer, arr, xy_coordinate)

        loss = evaluate(model, criterion, dataloader_valid, device, val_writer, xy_coordinate)
...
def parse_args():
    ...
    # Model related
    parser.add_argument('-m', '--model', default='DeepONet', type=str, help='inverse model name')
    ...
    parser.add_argument('-ns', '--N-sample', default=500, type=int)
