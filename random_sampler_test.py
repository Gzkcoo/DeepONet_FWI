...
def evaluate(model, criterions, dataloader, device, k, ctx,
             vis_path, vis_batch, vis_sample, missing, std, xy_loc):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for data, label in dataloader:
            ...
            if missing or std:
                ...
                pred = model(data, xy_locs)
            pred = pred.reshape(-1, 1, 70, 70)
...
def main(args):
    ...
    evaluate(model, criterions, dataloader_valid, device, args.k, ctx,
             vis_path, args.vis_batch, args.vis_sample, args.missing, args.std, xy_coordinate)
