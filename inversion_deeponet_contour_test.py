...
def evaluate(model, criterion, dataloader, device, k, ctx,
    ...
    contour_pred_tensor, contour_tensor = [], []
    ...
    with torch.no_grad():
        ...
        for data, source, label in dataloader:
            ...
            conlabels = torch.from_numpy(conlabels).to(device, non_blocking=True)
            ...
            pred1, pred2 = model(data, source)
            contour_pred_tensor.append(pred2)



            label_pred_np = T.tonumpy_denormalize(pred1, ctx['label_min'], ctx['label_max'], exp=False)
            label_pred_list.append(label_pred_np)
            label_pred_tensor.append(pred1)
            ...
            #         plot_velocity(contour_pred_soft[i, 1].cpu().numpy(), conlabels[i, 0].cpu().numpy(),
            #                       f'{vis_path}/V_{batch_idx}_{i}_contours.png')
    ...
    # print(f'Cross Entropy: {l3(contour_pred_t, torch.squeeze(contour_t).long())}')
