...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        batchsize = x.shape[0]   # x: (batchsize, width, 1000, 72)
        #Compute Fourier coeffcients up to factor of e^(- something constant)
        x_ft = torch.fft.rfft2(x)    # (batchsize, width, 1000, 37, 2)
...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2, width):
        ...
        self.modes2 = modes2
        self.width = width
        self.fc0 = nn.Linear(66, self.width)  # input channel is 3: (a(x, y), x, y)  \\  更改为64+2=66
    ...
    def forward(self, x):
        batchsize = x.shape[0]
        grid = self.get_grid(x.shape, x.device)  # (bacth, 1000, 72, 2)
        x = torch.cat((x, grid), dim=-1)         # (bacth, 1000, 72, 66)
        size_x, size_y = x.shape[1], x.shape[2]  # 1000, 72

        x = self.fc0(x)                          # (bacth, 1000, 72, width)
        x = x.permute(0, 3, 1, 2)                # (bacth, width, 1000, 72)

        x1 = self.conv0(x)                       # (bacth, width, 1000, 72)
        ...
        x = x.permute(0, 2, 3, 1)   # (bacth, 1000, 72, width)
        x = self.fc1(x)
        x = F.relu(x)
        # x = self.fc2(x)
        return x
...
class UFNO2d(nn.Module):
    def __init__(self, modes1, modes2,  width, padding=7): # I add
        ...
        self.modes1 = modes1   # 傅立叶层取前 modes 个模式
        self.modes2 = modes2
        self.width = width     # 网络内部层的神经元个数
        ...
        self.fc0 = nn.Linear(66, self.width)  # input channel is 3: (a(x, y), x, y)
        ...
        # UFNO的Linear  用来降维
        self.ufc1 = nn.Linear(1000, 512)
        self.ufc2 = nn.Linear(512, 256)
        self.ufc3 = nn.Linear(256, 70)

        self.fc1 = nn.Linear(self.width, 128)
        self.fc2 = nn.Linear(128, 1)

    def forward(self, x):                         # x : (batch, 1000, 72, 64)
        batchsize = x.shape[0]
        size_x, size_y = x.shape[1], x.shape[2]
        grid = self.get_grid(x.shape, x.device)   # (batch, 1000, 72, 2)
        x = torch.cat((x, grid), dim=-1)          # (batch, 1000, 72, 66)
        x = self.fc0(x)                           # (batch, 1000, 72, width)
        x = x.permute(0, 3, 1, 2)                 # (batch, width, 1000, 72)
        
        #print('b4 padding size of x',x.size())  
        #x = F.pad(x, [0,self.padding, 0,self.padding])  # 作用???

        
        #print('b4 1st fno layer of x',x.size())  
        x1 = self.conv0(x)         # (batch, width, 1000, 72)
        x2 = self.w0(x.view(batchsize, self.width, -1)).view(batchsize, self.width, 1000, 72)            # (batch, width, 1000, 72)
        x = x1 + x2                # (batch, width, 1000, 72)
        x = F.relu(x)     # (bacth, width, 1000, 72)
        ...
        x1 = self.conv3(x)
        x2 = self.w3(x.view(batchsize, self.width, -1)).view(batchsize, self.width, 1000, 72)
        x3 = self.unet3(x) 
        x = x1 + x2 + x3     # (batch, width, 1000, 72)

        x = x.permute(0, 1, 3, 2)   # (batch, width, 72, 1000)
        x = self.ufc1(x)            # (batch, width, 72, 512)
        x = x.permute(0, 1, 3, 2)   # (batch, width, 512, 72)
        x = F.relu(x)        
        
        x1 = self.conv4(x)
        x2 = self.w4(x.reshape(batchsize, self.width, -1)).reshape(batchsize, self.width, 512, 72)
        x3 = self.unet4(x)
        x = x1 + x2 + x3

        x = x.permute(0, 1, 3, 2)  # (batch, width, 72, 512)
        x = self.ufc2(x)  # (batch, width, 72, 256)
        x = x.permute(0, 1, 3, 2)  # (batch, width, 256, 72)
        x = F.relu(x)        
        
        x1 = self.conv5(x)
        x2 = self.w5(x.reshape(batchsize, self.width, -1)).reshape(batchsize, self.width, 256, 72)
        x3 = self.unet5(x)
        x = x1 + x2 + x3

        x = x.permute(0, 1, 3, 2)  # (batch, width, 72, 256)
        x = self.ufc3(x)  # (batch, width, 72, 70)
        x = x.permute(0, 1, 3, 2)  # (batch, width, 70, 72)
        x = F.relu(x)

        # x = x[..., :-self.padding, :-self.padding]  # 作用？？？
        x = x.permute(0, 2, 3, 1)  # (batch, 70, 72, 64)
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)     # (batch, 70, 72, 1)
        x = x.permute(0, 3, 1, 2)   # (batch, 1, 70, 72)

        x = x[:, :, :, 1:-1]  # (batch, 1, 70, 70)    Slicing
