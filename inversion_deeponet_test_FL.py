...
def l2_relative_loss(input, target):
    diff = input - target
    l2_norm_diff = torch.norm(diff, p=2, dim=(1, 2, 3))
    l2_norm_target = torch.norm(target, p=2, dim=(1, 2, 3))
    loss = l2_norm_diff / l2_norm_target
    return loss
...
def evaluate(model, criterion, dataloader, device, k, ctx,
    ...
    print(f'RE: {torch.mean(l2_relative_loss(label_t, pred_t))}')
...
def parse_args():
    ...
    parser.add_argument('-ds', '--dataset', default='flatvel-b-FL', type=str, help='dataset name')
    ...
    parser.add_argument('-v', '--val_anno', default='flatvel_b_val_FL.txt', help='name of val anno')
